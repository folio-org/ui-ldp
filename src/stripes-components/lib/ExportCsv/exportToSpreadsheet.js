...
function triggerDownload(blob, fileTitle, type, typeName) {
  const exportedFilename = (fileTitle || 'export') + '.' + type;
  if (navigator.msSaveBlob) { // IE 10+
    ...
    if (link.download !== undefined) { // feature detection
      ...
      link.setAttribute(`data-test-export${type}-link`, 'true');
      ...
      console.error(`Failed to trigger download for ${typeName} data`); // eslint-disable-line no-console
    }
  }
}
...
// Ignoring next block in tests since we don't have a great way to suppress downloads in tests
// istanbul ignore next
export default function exportToSpreadsheet(objectArray, opts, type, typeName, dataToBlob) {
  ...
  const blob = dataToBlob(fields, header);
  triggerDownload(blob, filename, type, typeName);
}
